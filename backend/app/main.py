...
class Item(BaseModel):
    ...
    R: float # 액면이자율(1년)
    YTM: float # 만기수익률(1년)
    m: int # 이자지급횟수(회1년 , ex  2 = 6개월마다 지급, 4 = 3개월마다 지급)
...
def calculate(FV: float, n: int, R: float, YTM: float, m: int):
    """
    # 이자의 연금현재가치계산과 원금의 현재가치계산을 합계하여 채권의 현재가치 출력
    """
    adjusted_R = R / m  # 주기별 이자율
    adjusted_YTM = YTM / m  # 주기별 수익률
    total_periods = n * m  # 총 이자 지급 횟수
